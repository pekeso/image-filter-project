...
(async () => {
  ...
  app.get( "/cars/:id", (req: Request, res: Response) => {
    ...
    let carId = parseInt(String(id));
    
    if (!carId) {
      return res.status(400).send(`ID is required ${carId}`);
    }
  ...
  app.post( "/cars/",
    async ( req: Request, res: Response) => {
      ...
      return res.status(201).send(`New car: {id: ${id}, make: ${make}, type: ${type}, model: ${model}, cost: ${cost}}`);
    })
